...
QUALITY_METRICS = ["answer_relevance", "faithfulness", "context_relevance", "recall@5", "mrr"]
    ...
    for attempt in range(_MAX_RETRIES):
        try:
            ...
            if is_rate_limit and attempt < _MAX_RETRIES - 1:
                ...
                print(f"         [rate limit] query {q.get('id','?')} — waiting {wait:.1f}s (attempt {attempt+1})", flush=True)
                time.sleep(wait)
                continue
            break

    return {
        "id": q.get("id", ""), "query": q.get("query", ""), "topic": "",
        "gold_doc_ids": [], "answer": None,
...
def run_config(
    ...
    with ThreadPoolExecutor(max_workers=workers) as pool:
        future_to_idx = {
            pool.submit(_run_query, q, vs, bm25, model, k, mode, use_bm25, do_rerank, do_rewrite): i
            for i, q in enumerate(queries)
        }
        for future in as_completed(future_to_idx):
            r = future.result()
            results.append(r)
            status = f"_error: {r['_error']}" if "_error" in r else f"ok  {r['elapsed_ms']}ms"
    ...
    # Aggregate
    def _avg_gen(key):
        vals = [r["generation_metrics"].get(key, {}).get("score")
                for r in results
    ...
    aggregate["composite"] = round(sum(quality_vals) / len(quality_vals), 4) if quality_vals else None
...
def _cfg_label(cfg: dict) -> str:
    ...
    return f"model={cfg['model']:<12}  k={cfg['k']:<2}  mode={cfg['mode']:<8}  {r:<9}  {w}"
...
def print_grid_summary(rows: list[dict]) -> None:
    ...
    print(f"  Composite: {_color(bagg['composite'])}{bagg['composite']}{RESET}  "
...
def main() -> None:
    ...
    parser.add_argument("--output-dir", type=Path,
                        default=Path("./data/eval_results/grid_search"))
    parser.add_argument("--no-save", action="store_true",
                        help="Skip saving per-run metrics to disk")
    ...
    print(f"\n{'='*60}")
    print(f"  RAG HYPERPARAMETER GRID SEARCH")
    print(f"{'='*60}")
    ...
    if output_dir:
        ...
        summary_sorted = sorted(summary, key=lambda r: r["aggregate"]["composite"] or 0, reverse=True)
