...
def main() -> None:
    ...
    parser.add_argument(
        "--filter-type", default=None, metavar="TYPE", help="PROCEDURE or CONCEPT"
    )
    parser.add_argument(
        "--retrieve-only",
        action="store_true",
    ...
    parser.add_argument("--verbose", action="store_true", help="Show retrieval scores")
    parser.add_argument(
        "--processed-dir",
    ...
    parser.add_argument(
        "--no-rewrite", action="store_true", help="Skip query rewriting"
    )
    parser.add_argument(
        "--no-hybrid", action="store_true", help="Vector search only (no BM25)"
    )
    parser.add_argument(
    ...
    from services.pipeline import _doc_to_source, build_vectorstore
    ...
    if not args.no_hybrid:
        ...
        if processed_dir.exists():
            ...
            print(
                f"Warning: processed dir {processed_dir} not found, falling back to vector-only.",
                file=sys.stderr,
            )
    ...
    if args.retrieve_only:
        ...
        if use_hybrid:
            ...
            docs = hybrid_search(
                args.question, vs, bm25, k_retrieve=50, k_final=args.n_results
            )
        else:
        ...
        for i, doc in enumerate(docs, 1):
            ...
            print(
                f"[{i}] [{src['content_type']}] {src['title']:<45} {src['source_dir']}{score_str}"
            )
    ...
    if args.verbose or args.show_pipeline:
        ...
        print(
            f"Retrieval:  BM25={n_bm25}, vector={n_vec} → reranked={n_rer}   ({r_ms}ms)"
        )
