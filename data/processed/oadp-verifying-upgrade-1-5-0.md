...
```
> **NOTE:** The `RECONCILED` column must be `True`.

1. Verify that the installation finished by viewing the OADP resources by running the following command:
```bash
...
```
> **NOTE:** The `node-agent` pods are created only while using `restic` or `kopia` in `DataProtectionApplication` (DPA). In OADP 1.4.0 and OADP 1.3.0 version, the `node-agent` pods are labeled as `restic`.
