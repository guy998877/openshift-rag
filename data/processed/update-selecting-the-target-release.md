...
Use the [Red Hat OpenShift Container Platform Update Graph](https://access.redhat.com/labs/ocpupgradegraph/update_path) or the
[cincinnati-graph-data repository](https://github.com/openshift/cincinnati-graph-data/tree/master/channels) to determine what release to update to.
...
> **NOTE:** You do not need to change the channel when performing a z-stream update.
...
1. Change the channel to point to the new channel you want to update to by running the following command:
```bash
$ oc adm upgrade channel eus-4.16
```

1. Confirm the updated channel by running the following command:
```bash
$ oc get clusterversion -o=jsonpath='{.items[*].spec}' | jq
```
.Example output
...
1. Check the update path from the new channel.
Run the following command:
```bash
$ oc adm upgrade
```
...
1. Follow the update procedure to get to version 4.16 (<y+1> from version 4.15)
> **NOTE:** You can keep your worker nodes paused between EUS releases even if you are using the fast channel.

1. When you get to the required <y+1> release, change the channel again, this time to `fast-<y+2>`.

1. Follow the EUS update procedure to get to the required <y+2> release.

## Changing the channel for a y-stream update

In a y-stream update you change the channel to the next release channel.

> **NOTE:** Use the stable or EUS release channels for production clusters.
