...
> **IMPORTANT:** If this command hangs, you might need to delete the finalizer by running the following command: [source,terminal] ---- $ oc -n openshift-adp patch cloudstorage ${CLUSTER_NAME}-oadp -p '{"metadata":{"finalizers":null}}' --type=merge ----
...
1. Remove the namespace from the Operator by running the following command:
```bash
