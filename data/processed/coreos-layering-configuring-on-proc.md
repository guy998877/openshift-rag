...
> **NOTE:** In a disconnected environment, ensure that the disconnected cluster can access the registry where you want to push the image. Image mirroring applies only to pulling images.
...
1. If necessary, when the `MachineOSBuild` object has been created and is in the `READY` state, modify the node spec for the nodes where you want to use the new custom layered image:
...
1. Verify the custom layered image build by running a command similar to the following:
```bash
...
1. Verify the current stage of your layered build by running the following command:
```bash
...
1. When the build is complete, verify that the image has been applied to the nodes in the affected pool by running a command similar to the following:
```bash
...
1. Verify that the appropriate nodes are using the new custom layered image:
