...
- Log in to link:https://console.redhat.com/openshift/assisted-installer/clusters[OpenShift Cluster Manager] as a user with cluster creation privileges.

- Install `jq`.

.Procedure

1. Log in to link:https://console.redhat.com/openshift/token/show[OpenShift Cluster Manager] and copy your API token.

1. Set the `$OFFLINE_TOKEN` variable using the copied API token by running the following command:
```bash
$ export OFFLINE_TOKEN=<copied_api_token>
```

1. Set the `$JWT_TOKEN` variable using the previously set `$OFFLINE_TOKEN` variable:
```bash
$ export JWT_TOKEN=$(
  curl \
  --silent \
...
```
> **NOTE:** The JWT token is valid for 15 minutes only.
