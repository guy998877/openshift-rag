...
> **IMPORTANT:** Manual provisioning of PVs includes the risk of potential data leaks across PV reuse when PVCs are deleted. The Local Storage Operator is recommended for automating the life cycle of devices when provisioning local PVs.
...
> **NOTE:** Do not use different storage class names for the same device. Doing so will create multiple PVs.
...
1. Create the PV resource in your OpenShift Container Platform cluster. Specify the file you just created:
```bash
