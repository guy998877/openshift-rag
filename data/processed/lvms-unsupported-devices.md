...
> **NOTE:** To get information about the devices, run the following command: [source,terminal] ---- $ lsblk --paths --json -o \ NAME,ROTA,TYPE,SIZE,MODEL,VENDOR,RO,STATE,KNAME,SERIAL,PARTLABEL,FSTYPE ----
...
> **IMPORTANT:** LVM Storage supports devices with `fstype` parameter set to `LVM2_member` only if the devices do not contain children devices.
...
> **NOTE:** It is recommended to wipe the device before using it in LVM Storage to prevent unexpected behavior.