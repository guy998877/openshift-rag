...
1. The control plane machine enters the `Deleting` phase.
1. To satisfy the `preDrain` lifecycle hook, the etcd Operator takes the following actions:
--
...
1. The control plane machine status condition `Drainable` is set to `True`.
1. The machine controller attempts to drain the node that is backed by the control plane machine.
...
1. The control plane machine status condition `Drained` is set to `True`.
1. If no other Operators have added a `preTerminate` lifecycle hook, the control plane machine status condition `Terminable` is set to `True`.
