...
```
> **TIP:** You can alternatively apply the following YAML to disable the default project-wide node selector for a namespace: [source,yaml] ---- apiVersion: v1 kind: Namespace metadata: name: <namespace> annotations: openshift.io/node-selector: '' #... ----
...
1. Create the daemon set object:
```bash
$ oc create -f daemonset.yaml
```

1. To verify that the pods were created, and that each node has a pod replica:
