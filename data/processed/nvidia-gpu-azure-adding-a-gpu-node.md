...
> **NOTE:** By default, Azure subscriptions do not have a quota for the Azure instance types with GPU. Customers have to request a quota increase for the Azure instance families listed above.
...
1. View the content of the machineset:
```bash
...
1. View the existing nodes, machines, and machine sets by running the following command. Note that each node is an instance of a machine definition with a specific Azure region and OpenShift Container Platform role.
```bash
$ oc get nodes
```
.Example output
...
> **NOTE:** There is no need to specify a namespace for the node. The node definition is cluster scoped.