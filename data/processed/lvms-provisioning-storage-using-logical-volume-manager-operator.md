...
> **NOTE:** The `volumeBindingMode` field of the storage class is set to `WaitForFirstConsumer`.

1. Create the PVC by running the following command:
```bash
# oc create -f <file_name> -n <application_namespace>
```
> **NOTE:** The created PVCs remain in `Pending` state until you deploy the pods that use them.
