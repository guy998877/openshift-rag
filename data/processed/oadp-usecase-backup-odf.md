...
1. When you create an OBC, ODF creates a `secret` and a `config map` with the same name as the object bucket claim. The `secret` has the bucket credentials, and the `config map` has information to access the bucket. To get the bucket name and bucket host from the generated config map, run the following command:
```bash
...
```
where:
`defaultSnapshotMoveData`:: Set to `true` to use the OADP Data Mover to enable movement of Container Storage Interface (CSI) snapshots to a remote object storage.
`s3Url`:: Specifies the S3 URL of ODF storage.
