...
> **WARNING:** Improper use of kernel arguments can result in your systems becoming unbootable.
...
> **WARNING:** Disabling SELinux on RHCOS in production is not supported. After SELinux has been disabled on a node, it must be re-provisioned before re-inclusion in a production cluster.
...
1. List existing `MachineConfig` objects for your OpenShift Container Platform cluster to determine how to
...
1. Create a `MachineConfig` object file that identifies the kernel argument (for example, `05-worker-kernelarg-selinuxpermissive.yaml`)
```yaml
...
1. Create the new machine config:
```bash
