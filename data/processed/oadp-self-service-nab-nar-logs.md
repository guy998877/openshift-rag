...
> **NOTE:** You can review the NAB logs only if you are using a `NonAdminBackupStorageLocation` (NABSL) CR as a backup storage location for the backup.
...
1. Verify that the NADR CR is processed by running the following command:
```bash
