...
`<existing_tag>`:: Specifies a previously pushed image tag, such as `pass:a[4.21]`.
`<updated_tag>`:: Specifies the image tag to apply to the updated index image, such as `pass:a[4.21].1`.
--
.Example command
```bash
$ opm index add \
    --bundles quay.io/ocs-dev/ocs-operator@sha256:c7f11097a628f092d8bad148406aa0e0951094a03445fd4bc0775431ef683a41 \
    --from-index mirror.example.com/abc/abc-redhat-operator-index:4.21 \
    --tag mirror.example.com/abc/abc-redhat-operator-index:4.21.1 \
    --pull-tool podman
```
