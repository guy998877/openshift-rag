...
1. If required, clean up the stored data on the `BackupStorageLocation` resource  to free up space.

1. Apply the `velero.io/csi-volumesnapshot-class=true` label to the `VolumeSnapshotClass` object by running the following command:
```bash
