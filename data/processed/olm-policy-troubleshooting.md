...
1. Check the status of the `InstallPlan` object for any errors:
```yaml
apiVersion: operators.coreos.com/v1
kind: InstallPlan
status:
...
> **NOTE:** Operator Lifecycle Manager (OLM) does not currently provide the complete list of errors on the first try.