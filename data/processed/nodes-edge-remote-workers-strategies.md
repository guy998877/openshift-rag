...
--
> **NOTE:** Daemon sets do not schedule pods after a reboot of the node if OpenShift Container Platform cannot reach the node.
...
> **NOTE:** Static pods cannot use secrets and config maps.
...
> **NOTE:** Modifying the `node-monitor-grace-period` parameter is not supported.
...
> **NOTE:** Unless tolerations are explicitly set, Kubernetes automatically adds a toleration for `node.kubernetes.io/not-ready` and `node.kubernetes.io/unreachable` with `tolerationSeconds=300`, meaning that pods remain bound for 5 minutes if either of these taints is detected.
...
> **NOTE:** When working with remote worker nodes, rescheduling pods on different nodes might not be acceptable if remote worker nodes are intended to be reserved for specific functions.
