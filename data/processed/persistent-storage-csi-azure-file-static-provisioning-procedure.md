...
1. Create a PV by using the following example YAML file:
...
1. Create a PVC that references the PV using the following example file:
...
1. Ensure that the PVC is created and in `Bound` status after a while by running the following command:
```bash
