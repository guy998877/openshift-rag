...
1. To add a node selector when creating a pod, edit the `Pod` configuration, and add
...
> **NOTE:** Labels can only be set to one value, so setting a node selector of `region=west` in a `Pod` configuration that has `region=east` as the administrator-set default, results in a pod that will never be scheduled.