...
> **NOTE:** * The password works only through a cloud provider serial console or a BMC. It does not work with SSH. * If you have a machine config that includes an `/etc/shadow` file or a systemd unit that sets a password, it takes precedence over the password hash.
...
```
1. Create a machine config file that contains the `core` username and the hashed password:
```bash
...
1. Check the contents of the `/etc/shadow` file:
.Example output
```bash
...
