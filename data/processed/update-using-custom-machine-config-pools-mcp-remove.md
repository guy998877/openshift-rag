...
> **IMPORTANT:** A node must have a role to be properly functioning in the cluster.

.Procedure

////
1. Ensure that the nodes have a `worker` label or a label from an MCP that is updated.
```bash
