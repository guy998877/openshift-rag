...
1. Drain. The MCO terminates all running workloads on the node, causing the workloads to be rescheduled onto other nodes.
1. Apply. The MCO writes the new configuration to the nodes as needed.
1. Reboot. The MCO restarts the node.
1. Uncordon. The MCO marks the node as schedulable for workloads.
...
> **NOTE:** In certain cases the nodes are not drained. For more information, see "About the Machine Config Operator."
