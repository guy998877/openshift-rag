...
1. Get cluster information for labels by running the following command:
```bash
...
1. Create a `Machine` object for the new control plane node by creating a yaml file similar to the following:
```yaml
...
1. Link the new control plane node and `Machine` object to the `BareMetalHost` object by performing the following steps in a single bash shell session:
...
1. Set the `BareMetalHost` object's `poweredOn` status to `true` by running the following command:
```bash
...
1. Review the `BareMetalHost` object's `poweredOn` status by running the following command:
```bash
$ oc get bmh -n openshift-machine-api -ojson | jq -r '.items[] | .metadata.name + "   PoweredOn:" +  (.status.poweredOn | tostring)'
```

1. Review the `BareMetalHost` object's provisioning state by running the following command:
```bash
$ oc get bmh -n openshift-machine-api -ojson | jq -r '.items[] | .metadata.name + "   ProvisioningState:" +  .status.provisioning.state'
```
> **IMPORTANT:** If the provisioning state is not `unmanaged`, change the provisioning state by running the following command: [source,terminal] ---- $ oc patch -n openshift-machine-api bmh $NEW_NODE_NAME --subresource status --type json -p '[{"op":"replace","path":"/status/provisioning/state","value":"unmanaged"}]' ----

1. Set the machine's state to `Provisioned` by running the following command:
```bash
