...
> **IMPORTANT:** Operators must handle setting environment variables for proxy settings in the pods for any managed Operands.
...
1. Select the Operator and click *Install*.

1. On the *Install Operator* page, modify the `Subscription` object to include one or more of the following environment variables in the `spec` section:
--
- `HTTP_PROXY`
- `HTTPS_PROXY`
- `NO_PROXY`
...
```
> **NOTE:** These environment variables can also be unset using an empty value to remove any previously set cluster-wide or custom proxy settings.
...
1. Click *Install* to make the Operator available to the selected namespaces.

1. After the CSV for the Operator appears in the relevant namespace, you can verify that custom proxy environment variables are set in the deployment. For example, using the CLI:
```bash
