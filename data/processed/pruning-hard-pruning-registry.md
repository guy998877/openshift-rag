...
1. *Switch the registry to read-only mode.*
...
1. *Add the `system:image-pruner` role.*
...
1. *Optional: Run the pruner in dry-run mode.*
...
1. *Switch the registry back to read-write mode.*
