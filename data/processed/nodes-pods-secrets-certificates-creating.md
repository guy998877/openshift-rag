...
1. Add the `service.beta.openshift.io/serving-cert-secret-name` annotation
...
1. View the secret to make sure it was created:
...
1. Edit your `Pod` spec with that secret.
```yaml
apiVersion: v1
kind: Pod
metadata:
...
RFC3339 format.
> **NOTE:** In most cases, the service DNS name `<service.name>.<service.namespace>.svc` is not externally routable. The primary use of `<service.name>.<service.namespace>.svc` is for intracluster or intraservice communication, and with re-encrypt routes.