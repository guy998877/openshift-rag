...
> **NOTE:** Changes to the route just change the portion of traffic to the various services. You might have to scale the deployment to adjust the number of pods to handle the anticipated loads.
...
1. Click the Options menu image:kebab.png[title="Options menu"] next to the route you want to edit and select *Edit Route*.
...
1. Run the command with no flags to verify the current configuration:
```bash
...
1. To override the default values for the load balancing algorithm, adjust the annotation on the route by setting the algorithm to `roundrobin`. For a route on OpenShift Container Platform, the default load balancing algorithm is set to `random` or `source` values.
...
> **NOTE:** Not all routers may support multiple or weighted backends.
...
1. Make the application available via a route, or use the service IP directly:
```bash
...
1. Browse to the application at `ab-example-<project_name>.<router_domain>` to verify you see the `v1` image.

1. Create a second shard based on the same source image and label as the first shard, but with a different tagged version and unique environment variables:
```bash
...
1. At this point, both sets of pods are being served under the route. However, because both browsers (by leaving a connection open) and the router (by default, through a cookie) attempt to preserve your connection to a back-end server, you might not see both shards being returned to you.
...
1. If you trigger a deployment on either shard, only the pods in that shard are affected. You can trigger a deployment by changing the `SUBTITLE` environment variable in either `Deployment` object:
```bash
