...
- A public client for the pass:quotes[OpenShift CLI (`oc`)] called `oc-cli-test` with the valid redirect URIs set to [x-]`http://localhost:8080`
...
<12> The client ID that your provider uses for the OpenShift CLI (`oc`).
<13> The client ID that your provider uses for the OpenShift Container Platform web console.
...
1. Verify that you can log in to the pass:quotes[OpenShift CLI (`oc`)] by authenticating with your identity provider:
...
1. Verify that you can log in to the OpenShift Container Platform web console by authenticating with your identity provider:
