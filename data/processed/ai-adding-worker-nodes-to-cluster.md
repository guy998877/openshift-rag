...
- Log in to link:https://console.redhat.com/openshift[OpenShift Cluster Manager] as a user with cluster creation privileges.
...
1. Set the `$API_URL` variable by running the following command:
```bash
$ export API_URL=<api_url>
```
Replace `<api_url>` with the Assisted Installer API URL, for example, `https://api.openshift.com`

1. Import the single-node OpenShift cluster by running the following commands:
...
1. Generate the `InfraEnv` resource for the cluster and set the `$INFRA_ENV_ID` variable by running the following commands:
.. Download the [pull secret from Red Hat OpenShift Cluster Manager](https://console.redhat.com/openshift/install/pull-secret).
...
`<path_to_pull_secret_file>`:: Specifies the path to the local file containing the downloaded [pull secret from Red Hat OpenShift Cluster Manager](https://console.redhat.com/openshift/install/pull-secret).
...
.. Post the `$INFRA_ENV_REQUEST` to the [/v2/infra-envs](https://api.openshift.com/?urls.primaryName=assisted-service%20service#/installer/RegisterInfraEnv) API and set the `$INFRA_ENV_ID` variable:
```bash
...
1. Get the URL of the discovery ISO for the cluster worker node by running the following command:
```bash
...
1. Boot the new worker host from the downloaded `rhcos-live-minimal.iso`.

1. Get the list of hosts in the cluster that are _not_ installed. Keep running the following command until the new host shows up:
```bash
...
1. Set the `$HOST_ID` variable for the new worker node, for example:
```bash
$ HOST_ID=<host_id>
```
Replace `<host_id>` with the host ID from the previous step.

1. Check that the host is ready to install by running the following command:
> **NOTE:** Ensure that you copy the entire command including the complete `jq` expression.
```bash
...
1. When the previous command shows that the host is ready, start the installation using the link:https://api.openshift.com/?urls.primaryName=assisted-service%20service#/installer/v2InstallHost[/v2/infra-envs/{infra_env_id}/hosts/{host_id}/actions/install] API by running the following command:
```bash
...
1. As the installation proceeds, the installation generates pending certificate signing requests (CSRs) for the worker node.
> **IMPORTANT:** You must approve the CSRs to complete the installation.
...
1. Optional: Run the following command to see all the events for the cluster:
```bash
...
1. Log in to the cluster and approve the pending CSRs to complete the installation.
