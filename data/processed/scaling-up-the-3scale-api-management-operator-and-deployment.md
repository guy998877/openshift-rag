...
1. Ensure that the 3scale pod is running to verify if the 3scale operator was deployed by running the following command:
```bash
...
1. Create the following script to scale up the deployments:
```bash
...
1. Scale up the deployments by running the following script:
```bash
$ ./scaledeployment.sh
```
.Example output
...
1. Get the `3scale-admin` route to log in to the 3scale UI by running the following command:
```bash
...
1. Use the URL from this output to log in to the 3scale operator as an administrator. You can verify that the data, when you took backup, is available.