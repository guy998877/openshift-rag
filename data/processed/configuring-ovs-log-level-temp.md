...
1. View the current syslog level for OVS modules:
```bash
...
1. Specify the log level in the `/etc/systemd/system/ovs-vswitchd.service.d/10-ovs-vswitchd-restart.conf` file:
```text
Restart=always
