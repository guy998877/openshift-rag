...
1. Verify the Ignition file configuration.
...
1. Verify the availability of the bootstrap node's assigned storage device.

1. Verify that the bootstrap node has been assigned an IP address from the DHCP server.

1. Collect `bootkube.service` journald unit logs from the bootstrap node. Replace `<bootstrap_fqdn>` with the bootstrap node's fully qualified domain name:
```bash
...
1. Collect logs from the bootstrap node containers.
...
1. If the bootstrap process fails, verify the following.
