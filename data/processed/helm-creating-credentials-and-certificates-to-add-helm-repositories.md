...
-n openshift-config
```
1. In the `openshift-config` namespace, create a `Secret` object to add the client TLS configurations:
```bash
...
1. Add the Helm repository as follows:
```bash
$ cat <<EOF | oc apply -f -
...
1. By default, all authenticated users have access to all configured charts. However, for chart repositories where certificates are needed, you must provide users with read access to the `helm-ca-cert` config map and `helm-tls-configs` secret in the `openshift-config` namespace, as follows:
```bash
