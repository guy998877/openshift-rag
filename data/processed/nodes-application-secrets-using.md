...
1. Save the following YAML example to a file named `service-account-token-secret.yaml`. The example includes a `Secret` object configuration that you can use to generate a service account token:
```yaml
apiVersion: v1
kind: Secret
metadata:
...
1. Generate the service account token by applying the file:
```bash
$ oc apply -f service-account-token-secret.yaml
```

1. Get the service account token from the secret by running the following command:
```bash
...
1. Use your service account token to authenticate with the API of your cluster:
```bash
