...
1. Create the second `Secret` for MCG with a custom name by running the following command:
```bash
...
1. Configure the DPA with the two BSLs as shown in the following example.
.Example DPA
```yaml
...
1. Verify that the DPA has reconciled by running the following command:
```bash
$ oc get dpa -o yaml
```

1. Verify that the BSLs are available by running the following command:
```bash
$ oc get bsl
```
.Example output
...
1. Create a backup CR with the default BSL. 
> **NOTE:** In the following example, the `storageLocation` field is not specified in the backup CR.
.Example backup CR
```yaml
...
1. Verify that the backup completed with the default BSL by running the following command:
```bash
...
1. Create a backup CR by using MCG as the BSL. In the following example, note that the second `storageLocation` value is specified at the time of backup CR creation.
.Example backup `CR`
```yaml
apiVersion: velero.io/v1
kind: Backup
...
1. Create a second backup by running the following command:
```bash
$ oc apply -f <backup_file_name>
```
where:
...
1. Verify that the backup completed with the storage location as MCG by running the following command:
```bash
$ oc get backups.velero.io <backup_name> -o yaml
```
where:
