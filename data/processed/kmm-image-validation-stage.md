...
> **NOTE:** In case a `build` section exists, the input image in the `sign` section is used as the output image by the `build` section. Therefore, in order for the input image to be available to the `sign` section, the `PushBuiltImage` flag must be defined in the `PreflightValidationOCP` CR.