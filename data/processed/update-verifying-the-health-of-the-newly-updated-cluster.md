...
1. Check that all nodes are ready:
```bash
$ oc get nodes
```
.Example output
...
1. Check that there are no paused `mcp` resources in the cluster:
```bash
...
1. Check that all cluster Operators are available:
```bash
$ oc get co
```
.Example output
...
1. Check that all pods are healthy:
```bash
$ oc get po -A | grep -E -iv 'complete|running'
```
This should not return any pods.
> **NOTE:** You might see a few pods still moving after the update. Watch this for a while to make sure all pods are cleared.