...
> **NOTE:** OADP with Azure STS configuration does not support `restic` File System Backups (FSB) and restores.

.Prerequisites

- You have an OpenShift cluster installed on Microsoft Azure with Microsoft Entra Workload ID configured. For more details see, link:https://docs.redhat.com/en/documentation/openshift_container_platform/4.21/html/installing_on_azure/installer-provisioned-infrastructure#installing-azure-with-short-term-creds_installing-azure-customizations[Configuring an Azure cluster to use short-term credentials].
...
> **NOTE:** If your OpenShift cluster was not originally installed with Microsoft Entra Workload ID, you can enable short-term credentials after installation. This post-installation configuration is supported specifically for Azure clusters.
...
> **IMPORTANT:** After enabling Microsoft Entra Workload ID on an existing Azure cluster, you must update all cluster components that use cloud credentials, including OADP, to use the new authentication method.

1. Set the environment variables for your Azure STS configuration as shown in the following example:
```bash
...
1. Create an Azure Managed Identity for OADP as shown in the following example:
```bash
...
1. Grant the required Azure roles to the managed identity as shown in the following example:
```bash
...
1. Create an Azure storage account and a container as shown in the following example:
```bash
...
1. Get the OIDC issuer URL from your OpenShift cluster as shown in the following example:
```bash
...
1. Configure Microsoft Entra Workload ID Federation as shown in the following example:
```bash
...
1. Create the OADP namespace if it does not already exist by running the following command:
```bash
oc create namespace openshift-adp
```

1. To use the `CloudStorage` CR to create an Azure cloud storage resource, run the following command:
```bash
cat <<EOF | oc apply -f -
...
1. Create the `DataProtectionApplication` (DPA) custom resource (CR) and configure the Azure STS details as shown in the following example:
```bash
cat <<EOF | oc apply -f -
apiVersion: oadp.openshift.io/v1alpha1
...
1. Create a backup of an application and verify the backup is stored successfully in Azure storage.