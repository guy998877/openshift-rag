...
1. Create a local CA key and certificate and use them to sign the webhook server's certificate signing request (CSR).

1. Create a new project for webhook resources:
```bash
$ oc new-project my-webhook-namespace  <1>
```
<1> Note that the webhook server might expect a specific name.

1. Define RBAC rules for the aggregated API service in a file called `rbac.yaml`:
```yaml
apiVersion: v1
kind: List
items:
...
1. Apply those RBAC rules to the cluster:
```bash
$ oc auth reconcile -f rbac.yaml
```

1. Create a YAML file called `webhook-daemonset.yaml` that is used to deploy a webhook as a daemon set server in a namespace:
```yaml
apiVersion: apps/v1
kind: DaemonSet
metadata:
...
1. Deploy the daemon set:
```bash
$ oc apply -f webhook-daemonset.yaml
```

1. Define a secret for the service serving certificate signer, within a YAML file called `webhook-secret.yaml`:
```yaml
apiVersion: v1
kind: Secret
metadata:
...
1. Define a service account and service, within a YAML file called `webhook-service.yaml`:
```yaml
apiVersion: v1
kind: List
items:
...
1. Expose the webhook server within the cluster:
```bash
$ oc apply -f webhook-service.yaml
```

1. Define a custom resource definition for the webhook server, in a file called `webhook-crd.yaml`:
```yaml
...
1. Configure the webhook server also as an aggregated API server, within a file called `webhook-api-service.yaml`:
```yaml
...
1. Deploy the aggregated API service:
```bash
$ oc apply -f webhook-api-service.yaml
```

1. Define the webhook admission plugin configuration within a file called `webhook-config.yaml`. This example uses the validating admission plugin:
```yaml
...
1. Verify that the webhook is functioning as expected. For example, if you have configured dynamic admission to reserve specific namespaces, confirm that requests to create those namespaces are rejected and that requests to create non-reserved namespaces succeed.