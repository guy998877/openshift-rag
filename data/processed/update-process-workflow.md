...
1. The CVO detects that the `desiredUpdate` field in the `ClusterVersion` resource differs from the current cluster version.
...
1. The CVO validates the integrity and authenticity of the release image.
...
1. The CVO creates a job named `version-$version-$hash` in the `openshift-cluster-version` namespace.
...
1. The CVO validates the extracted manifests and metadata.

1. The CVO checks some preconditions to ensure that no problematic condition is detected in the cluster.
...
1. The CVO records the accepted release in `status.desired` and creates a `status.history` entry about the new update.

1. The CVO begins reconciling the manifests from the release image.
...
1. Manifests for the CVO itself are applied early in the process.
...
1. The update proceeds until the entire control plane is updated to the new version.
...
1. The Machine Config Operator (MCO) manifests are applied towards the end of the process.
